...
public class AsyncOrderMealList extends AsyncTask<String, Void, MealWrapper> {
    MealWrapper hilf;

    public AsyncOrderMealList(MealWrapper a) {
        super();
        hilf = a;
    }

    @Override
    protected MealWrapper doInBackground(String... params) {
        return orderCart(hilf);
    }

    private MealWrapper orderCart(MealWrapper hilf) {
        URL url;
        try {
            url = new URL("http://192.168.75.1:18080/HolidayOutServer/webresources/ordermeal");
